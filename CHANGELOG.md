...
### 单源码收口
- 本仓库改为稳定的 Lite Pages 入口，不再保存 Full 仓库的 JS/CSS 副本
- Lite 入口统一跳转到 `Geomap-app/?variant=lite`
- 新增入口回归测试和持续集成
- 补充 MIT 许可证
...
#### ⚙️ 弹出信息字段自定义
- **新增**: "弹出信息配置"入口（高级工具面板）
- **功能**: 点击配置后可自定义标记点击弹出气泡中显示的字段
- **内置字段**: 名称、类型、地址、经纬度（可开关，不可删除）
- **自定义字段**: 输入字段键名 + 显示标签，即可新增任意字段
- **持久化**: 配置存储于 localStorage，刷新页面不重置
- **一键刷新**: 保存后立即刷新所有地图标记的 popup 内容
...
### 🧹 项目整理
- **删除**: `map-app-single单网页版.html`、`user_example.geojson`、`便携版打包指南.md`、`打包便携版.bat`、`GitHub部署指南.md`、`SYNC_GUIDE.md`、`PRODUCT_PLAN_P0.md`、`使用说明-面板控制.md`、`快速启动.html`（共9项过时文件）
...
#### 🔧 新增 build-single.py
- **新增**: `python build-single.py` 生成空白版单网页
- **新增**: `--with-data <geojson>` 支持嵌入数据，`--output` 支持自定义文件名
- **输出**: `地图编辑器-空白版.html` / `地图编辑器-数据版.html`
